...
1. The estimation of a non-parametric two-component mixture model to identify substitution frequencies most likely to be affected by the experimental procedure
2. The identification of binding sites (clusters).
...
**wavClusteR** expects a sorted and index BAM file as input. A streamlined workflow to generate the required input from a fastq file is outlined below (line 1 is pseudo code, replace it with the aligner specific syntax).
...
1. Short read alignment to the reference genome using relaxed alignment parameters. Experimentally-induced transitions will otherwise hamper alignment of informative reads. Currently, **wavClusteR** has been tested with bowtie/Bowtie2 [Langmead et al., 2009](http://www.ncbi.nlm.nih.gov/pubmed/19261174); [Langmead and Salzberg, 2012](http://www.ncbi.nlm.nih.gov/pubmed/22388286)
2. Conversion of the alignment file from SAM to BAM format, e.g. using the ```samtools view``` utility from [SAMtools](http://samtools.sourceforge.net)
3. Sorting of the BAM file, e.g. using ```samtools sort```
4. Indexing of the sorted BAM file, e.g. with ```samtools index```.
...
**wavClusteR** provides a chunk of a human Argonaute 2 (AGO2) PAR-CLIP data set from [Kishore et al., 2011](http://www.ncbi.nlm.nih.gov/pubmed/?term=kishore+2011+argonaute). Reads in the chunk map to the genomic interval [chrX:23996166-24023263]. This data set is used in the following sections to illustrate a workflow for PAR-CLIP data analysis.
